    ...
    <div class="p-4 sm:ml-64">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('berita.update', $berita->slug) }}" enctype="multipart/form-data">
                    ...
                    <select name="kategori_id" id="kategori"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->kategori_id }}"
                                {{ old('kategori_id', $berita->kategori_id) == $category->kategori_id ? 'selected' : '' }}>
                                {{ $category->kategori }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    {{-- bagian untuk menampilkan foto yang telah diupload sebelumnya --}}
                    {{-- @if ($berita->photo)
                        <div class="mb-6">
                ...
                <div class="mb-6">
                    <label for="related_image">Edit detail Foto</label>
